...
class PinSpec_2560 : public PinSpec
{
	...
	public:
		...
		std::string GetMCUName() const override { return Config::Get().GetDebugCore()?"atmega404":m_strMCU; };
	...
	private:
		...
		const unsigned char DPin2Mask[86] = {
			// PIN IN PORT
			// -------------------------------------------
		...
		const unsigned char DPin2Port[86] = {
			// PORTLIST
			// -------------------------------------------
			PE	, // PE 0 ** 0 ** USART0_RX
			PE	, // PE 1 ** 1 ** USART0_TX
