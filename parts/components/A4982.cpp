...
void A4982::Draw()
{
        ...
        glPushMatrix();
            ...
            glScalef(0.09,-0.05,0);
			std::string strPos = to_string(m_fCurPos);
			strPos.resize(6);
            for (auto &c : strPos)
...
void A4982::OnEnableIn(struct avr_irq_t * irq, uint32_t value)
{
    if (irq->value == value && m_bEnable == (value==0))
        return;
	TRACE(cout << "A4982 " << m_cAxis.load() << " ENABLE:" << m_bEnable << '\n');
...
void A4982::OnMSIn(avr_irq_t *irq, uint32_t value)
{
	...
	switch (m_uiNewShift)
	{
		case 0:
			...
			m_iCurStep = ((unsigned) m_iCurStep >> 4U) << 4U;
			break;
		case 1:
			m_uiStepSize = 8; // half-step
			m_iCurStep = ((unsigned) m_iCurStep >> 3U) << 3U;
			break;
		case 2:
			m_uiStepSize = 4;
			m_iCurStep = ((unsigned) m_iCurStep >> 2U) << 2U;
			break;
		case 3:
			m_uiStepSize = 1;
...
void A4982::Init(struct avr_t * avr)
{
	...
	pTH->AddTrace(this, MS1_IN,{TC::OutputPin, TC::Stepper});
	pTH->AddTrace(this, MS2_IN,{TC::OutputPin, TC::Stepper});
	pTH->AddTrace(this, RESET_IN,{TC::OutputPin, TC::Stepper});
	pTH->AddTrace(this, SLEEP_IN,{TC::OutputPin, TC::Stepper});
	pTH->AddTrace(this, MAX_OUT,{TC::InputPin, TC::Stepper});
	pTH->AddTrace(this, MIN_OUT,{TC::InputPin, TC::Stepper});
}

float A4982::StepToPos(int32_t step)
{
	// Position is always in 16ths of a step.
	return (float)step/16.f/(float)m_cfg.uiStepsPerMM;
}

int32_t A4982::PosToStep(float pos)
{
	// Position is always 16ths of a step...
	return pos*16.f*(float)m_cfg.uiStepsPerMM;
}
