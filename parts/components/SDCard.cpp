...
static uint8_t CRC7(gsl::span<uint8_t> data)
{
	...
	for (auto &c: data) {
		crc ^= c;
		for (j = 0; j < 8; j++) {
			crc = (crc & 0x80u) ? gsl::narrow_cast<uint8_t>((gsl::narrow<unsigned>(crc << 1U) ^ gsl::narrow<unsigned>(poly << 1U))) : gsl::narrow_cast<uint8_t>((crc << 1U));
		}
	}
	return crc | 0x01U;
}
...
void SDCard::Reset()
{
	RaiseIRQ(CARD_PRESENT, !m_bMounted);
}
