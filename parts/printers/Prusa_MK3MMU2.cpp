/*
	Prusa_MK3MMU2.cpp - Printer definition for the Prusa MK3 w/MMU2
...
#include "printers/Prusa_MK3.h"  // for Prusa_MK3
#include "uart_pty.h"             // for uart_pty

Prusa_MK3MMU2::~Prusa_MK3MMU2()
{
	delete m_pipe;
}

void Prusa_MK3MMU2::SetupHardware()
{
	Prusa_MK3S::SetupHardware();
	TryConnect(MMU_HWRESET,m_MMU,MMU2::RESET);
	IR.Set(IRSensor::IR_AUTO);
	avr_irq_register_notify(m_MMU.GetIRQ(MMU2::FEED_DISTANCE), MAKE_C_CALLBACK(Prusa_MK3MMU2,OnMMUFeed),this);
...
void Prusa_MK3MMU2::OnVisualTypeSet(VisualType type)
{
...
std::pair<int,int> Prusa_MK3MMU2::GetWindowSize()
{
...
void Prusa_MK3MMU2::OnMMUFeed(struct avr_irq_t * irq, uint32_t value)
{
...
void Prusa_MK3MMU2::OnKeyPress(unsigned char key, int x, int y)
{
	switch (key) {
		case 'F':
		{
